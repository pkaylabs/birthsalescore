...
# def can_send_vt(request, amount: Decimal, user: None) -> bool:
#     '''
...
#     can_transfer = user_wallet.can_transfer_funds(amount)  # if funds is more than 0
#     return can_transfer


# def can_cashout(request, user: None, amount: Decimal):
#     '''
#         checks if the user can cashout: account is > 0 and account is > cashout amount.
#         parse [user] in case of a ussd request since they are not autheticated.
#     '''
...
#     print("Checking transaction type: ", type)
#     if type == TransactionType.CASHOUT_VT.value:
#         destination_account = phone
#         source_wallet = wallet  # noqa
#         destination_wallet = None  # no need for destination wallet when cashing out
#         source_account = None  # no need for source account when cashing out
#     elif type == TransactionType.TOPUP_VT.value:
#         destination_wallet = wallet  # noqa
#         source_account = phone
#         destination_account = None  # not needed when topping up
#         source_wallet = None    # not needed when topping up

#     print("generating transaction id")
#     transaction_id = generate_transaction_id()  # generate a unique transaction id
#     data = {
#         'transaction_id': transaction_id,
#         'mobile_number': destination_account if type == TransactionType.CASHOUT_VT.value else source_account,  # noqa
#         'amount': amount,
